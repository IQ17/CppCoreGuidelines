...
哲理性的规则通常都无法通过死板的规则检查.
但每一条反应这些哲理性主题的规则却可以这么来检查.
没有哲理性的根基的话, 更加具体的规则便会缺乏依据.

### <a name="Rp-direct"></a> P.1:  直接用代码表达想法

##### Reason

编译器不会读注释(或者是设计文档), 大部分程序员也一样.
代码中表达的内容直接定义了语意并且(在原则上)可以通过使用编译器和其他工具对其进行检查.
...
第一个 `month` 声明中显示地表达了要返回一个 `Month` 并且不会修改 `Date` 对象的状态.
第二个版本不得不让读者猜测这段代码的意图并且留下了出现 bug 的可能.
...
一个设计良好的库会比直接使用某些语言特性更好地表达意图(要做什么, 而不是怎么做的).

C++ 程序员应该对标准库的有基础的了解, 并且知道在合适的地方使用它.
任何程序员都应该知道她项目中用到的基础库, 并且做到合理的运用.
用到本指南的程序员应该了解 [guideline support library](#S-gsl), 并做到合理运用.
...
更好的做法是显示地表明 double 的含义(新的速度值, 还是速度变化量Δ, 还是旧的速度值?)和单位:
...
我们本可以用一个 `double` 值(不带单位)来表示Δ, 但这样会容易出错.
如果我们既想要绝对的速度值又想使用Δ, 我们可以定义一个 `Delta` 类型.

##### Enforcement

通常非常困难.

* 总是使用 `const` (检查成员函数是否修改对象状态; 检查函数是否修改通过指针或者引用传入的参数)
* 标明使用 cast 的地方(cast 会阉掉类型系统的检查)
* 检测模仿标准库功能的代码(很难做到)
...
除非某些代码的意图被表述出来(例如通过名字或者注释), 想要知道这些代码是做什么的基本是不可能的.
...
现在, 代码中没有显示的提及遍历容器的机制, 并且通过 const 引用来遍历元素, 这样就不会意外地修改元素. 如果想要进行修改操作, 就这么写:
...
最后一种写法清楚地表明了我们并不在乎用什么顺序对 `v` 中的元素进行处理.
...
另一种表述: 说明要做什么, 而不是怎么做这件事.
(译注: 代码应该更具描述性, 这可以通过合理使用你的基础库中提供的各种良好命名的函数原语, 来清楚的表达你的意图.
软件系统中的各种函数是具有层次结构的, 处于抽象底层的函数可能会更关注细节, 而抽象中处于上层的函数通过使用比其抽象层次更低的各种函数来表达当前函数的意图.
大部分底层的抽象可能都在标准库或者基础库中已经有提供了, 所以程序员大部分时间都是在编写抽象上层的函数.
当你在编码时, 应该保持清楚的思路, 明确自己当前编写的函数处于抽象的哪一层.
这样当读者阅读你的代码时, 就更容易从代码中了解到你的意图.)

##### Note

有些语言构造相对于其他的语言构造可以更好地表达意图.

##### Example

如果用两个 `int` 代表着二维坐标, 这么写:
...
* 简单 `for` 循环 vs. range-`for` 循环
* `f(T*, int)` 接口 vs. `f(span<T>)` 接口(译注: `span<T>` 是什么, 看GSL)
* 循环变量的作用范围太大
* 裸的 `new` 和 `delete`
* 函数中有太多的内置类型的参数
...
* unions - 用 `variant`
* casts - 尽量减少使用, 可以用模板来帮助做到这点
* array decay - 用 `span`
* range errors - 用 `span`
* narrowing conversions - 少用并且在必要时使用 `narrow` 或者 `narrow_cast`

### <a name="Rp-compile-time"></a> P.5: 倾向使用编译期检查而不是运行时检查

##### Reason

代码清晰度和性能. 如果错误能在编译期就发现, 你就不需要再写错误处理函数.
...
### <a name="Rp-run-time"></a> P.6: 编译期检查不了的东西需要在运行时可检查

##### Reason

放着难检测的错误在程序里不管简直是自找崩溃.

##### Note

理想的话, 我们可以在编译期或者运行时捕获所有的错误(不包括程序员写的逻辑错误). 编译期不可能查到所有错误, 而运行时检查所有错误成本又太高. 但是我们应该努力让我们的程序从原则上来讲可以被检查, 当然前提是如果有足够的资源(分析程序, 运行时检查, 机器资源, 时间).
...
这里有一点关键的信息(元素个数)非常的含糊不清, 以至于静态分析几乎不可行, 而当 `f()` 是ABI的一部分的时候, 又不能去在指针里动手脚, 这就导致动态检查很难做. 上面这个函数的设计让错误检查变得非常困难.

##### Example, bad

当然我们可以把元素个数也作为参数和指针一同传给函数:
...
同时传递元素个数比只传指针给函数会强不少(也更常见). 但是, 只要手一抖打错字里, 这里就会出现个严重的错误.

另外, 这里 `f2()` 该不该 `delete` 它的参数呢?

##### Example, bad

标准库中的资源管理指针也没能传递元素的个数:
...
这个设计将元素个数作为对象不可或缺的一部分, 这样就不容易出错, 而且运行时检查也总是可行的.

##### Example

如何转移所有权以及和验证用法正确与否有关的信息呢?
...
### <a name="Rp-early"></a> P.7: 及早的捕获运行时错误

##### Reason

避免"莫名其妙"的崩溃.
避免会导致(可能无法检查出来的)错误结果的错误.
...
这里我们在 `use1` 翻了个小错误, 这就会导致数据损坏或者程序崩溃.
(指针, 数量)风格的接口让 `increment1()` 没法防御越界错误.
假设我们可以通过检查下标防止越界访问, 那错误只有在调用到 `p[10]` 时候才会被发现.
我们可以尽早检查并且改善代码:
...
现在, `m<=n` 可以尽早的检查出来.
如果我们确实是打错字了, 本来是想用 `n` 的, 代码可以进一步简化(完全消除了错误的可能性):
...
数据被验证了两次(通过 `Date` 的构造函数)并且被当做了字符串传递(无结构数据).

##### Example

过度检查可能会很耗.
有些时候检查的太早可能会很傻, 因为你可能根本就用不到那个值, 或者你只需要值的一部分. 类似地, 不要让检查过程改变你函数接口的行为(比如添加了一个 `O(n)` 的检查, 而接口的平均复杂度是 `O(1)`).
...
对喷气机来讲, 物理定律可能不是一个不变量, 因为有可能会有测量误差.
...
### <a name="Rp-leak"></a> P.8: 别泄漏任何资源

##### Reason

即使资源消耗速度很慢, 长期来看, 所有的可用资源也会被消耗完. 这对于长期运行的程序尤为重要, 同样也是作为一个负责任的程序员的必要条件.
...
通俗来讲, 泄漏指的就是"任何没清理干净的东西". 正式点说就是任何"无法再被清理的东西". 例如在堆上分配了内存, 又失去了这块内存的地址, 那就没法再释放这块内存了.
...
### <a name="Rp-waste"></a> P.9: 不要浪费时间或者空间
...
没错, 上面的代码示例有点夸大问题的意思, 但每一个单独的错误我们都在产品代码中见过, 可能实际情况还更糟.
注意 `X` 的布局保证至少浪费了6字节的空间.
错误的拷贝操作定义废掉了移动语义, 所以函数返回值会很慢.
用 `new` 和 `delete` 分配 `buf` 简直多余; 如果我们真想要一个局部的字符串, 那就该直接分配个局部 `string`.
还有一些其他的性能bug和无谓的增加复杂度的问题.

##### Note

单独的一个浪费可能不怎么明显, 而且如果很明显, 可能一个专家级程序员很容易就会消除掉这个问题.
但是, 随意的在代码库中造成各种浪费很容易造成明显的问题, 而专家们又不是像我们想的那么闲.
这条规则的目标是直接避免大部分的浪费而不是等发生了再说.
而后, 我们就可以关注跟算法和需求有关的浪费, 但这个和此指南的关系不大.
